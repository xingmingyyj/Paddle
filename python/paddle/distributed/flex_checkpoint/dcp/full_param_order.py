...
class BaseAssembler(abc.ABC):
    ...
    def _prepare_metainfo(self, source_state_shard_info):
        ...
        output_vars = self.split_dict_by_key(self.aoa_engine.output_vars)
    ...
    def split_dict_by_key(self, data_dict):
...
class HVCommGroupFullParamAssembler(BaseAssembler):
    ...
    def get_batch_read_items(
        self, read_items, vertical_ranks, horizontal_index
    ):
    ...
    def _process_one_batch(self) -> list[str]:
        ...
        batch_items, batch_indices, op_type = self.get_batch_read_items(
            self.read_items, self.vertical_ranks, self.horizontal_index
        )
